...
function ContactWithSalesTeam() {
    ...
    const salesTeamValidationSchema = yup.object({
    ...
    const formik = useFormik({
        ...
        validationSchema: salesTeamValidationSchema,
        onSubmit: async(values , {resetForm})=>{
            try {
                const response = await axios.post('http://127.0.0.1:8000/api/v1/sales-team-requests' , values);
                Swal.fire({
    ...
    return (
        <div className="min-h-screen">
            <Helmet>
                <title>{lang === 'en' ? 'DJ-MOTORS | CONTACT WITH SALES TEAM' : 'دي جي موتورز | التزاصل مع فريق المبيعات'}</title>
            </Helmet>

            <Sectionheading heading={t('ContactWithSalesTeamHeading')} textSize={'lg:text-4xl text-3xl'} />
...
export default ContactWithSalesTeam;