...
function ContactForComplaint() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async(values , {resetForm})=>{
            try {
                    const response = await axios.post('http://127.0.0.1:8000/api/v1/complaint-requests' , values);

                    Swal.fire({
                        title: t('titleOfSuccessComplaintRequestMsg'),
                        icon: 'success',
    ...
    return (
        ...
        <Helmet>
            <title>{lang === 'en' ? 'DJ-MOTORS | Complaints' : 'دي جي موتورز | الشكاوى '}</title>
        </Helmet>

        <Sectionheading heading={t('ContactForComplaintHeading')} textSize={'lg:text-4xl text-3xl'} />
        ...
        <div className='sm:max-w-4xl md:max-w-5xl lg:max-w-7xl mx-auto my-8'>
            <form action="" method='post' onSubmit={formik.handleSubmit}>
                <div className='sm:grid sm:grid-cols-2 grid-cols-1 justify-center items-center gap-4'>
                    <div className='w-full flex flex-col gap-4 p-4 order-1'>
...
export default ContactForComplaint;